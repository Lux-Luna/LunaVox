...
def _load_all_tensors(model_dir: Path) -> dict[str, torch.Tensor]:
    ...
    for sf in sorted(model_dir.glob("*.safetensors")):
        with safe_open(sf, framework="pt", device="cpu") as f:
            for k in f.keys():  # noqa: SIM118  (safe_open exposes .keys(), not __iter__)
...
def export_embeddings(input_dir: Path, output_dir: Path) -> None:
    ...
    missing = [
        k for k in [text_key, fc1_w_key, fc1_b_key, fc2_w_key, fc2_b_key] if k not in tensors
    ]
    if missing:
    ...
    np.save(
        output_dir / "codec_embedding_0.npy",
        tensors[codec0_key].float().cpu().numpy().astype(codec_dtype),
    )
    ...
    for i in range(15):
        ...
        np.save(
            output_dir / f"codec_embedding_{i + 1}.npy",
            tensors[k].float().cpu().numpy().astype(codec_dtype),
        )
    ...
    for proj_w_key, proj_b_key in proj_candidates:
        if proj_w_key in tensors and proj_b_key in tensors:
            np.save(
                output_dir / "proj_weight.npy",
                tensors[proj_w_key].float().cpu().numpy().astype(proj_dtype),
            )
            np.save(
                output_dir / "proj_bias.npy",
                tensors[proj_b_key].float().cpu().numpy().astype(proj_dtype),
            )
            break
...
def main() -> None:
    ...
    parser.add_argument(
        "--input", "-i", type=Path, required=True, help="Path to HF base model directory"
    )
    parser.add_argument(
        "--output", "-o", type=Path, required=True, help="Output embeddings directory"
    )
