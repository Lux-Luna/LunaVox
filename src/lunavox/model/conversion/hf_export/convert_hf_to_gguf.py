...
# -*- coding: utf-8 -*-
#
# LunaVox-owned derivative of llama.cpp convert_hf_to_gguf.py.
#
# Originally vendored from ggml-org/llama.cpp and trimmed in 2026-04 from
# ~11,433 LOC to ~2,400 LOC by removing every non-Qwen architecture class
# and every non-text code path (Mistral-format, mmproj, vision encoders,
# audio encoders, MoE variants other than those on the Qwen3 inheritance
# chain). The surviving scope is exactly what the LunaVox conversion
# pipeline exercises:
#
#   - Dispatch: Qwen3ForCausalLM  ->  Qwen3Model
#                                      |
#                                      v
#                                     Qwen2Model  ->  TextModel  ->  ModelBase
#
#   - Vocab: the tiktoken BPE helpers on QwenModel (kept as a static-method
#     namespace) and TextModel._set_vocab_sentencepiece / _set_vocab_gpt2.
#
#   - Main entry point: main() still dispatches by architecture string, so
#     calling this file with --mistral-format or --mmproj now raises
#     NotImplementedError rather than silently crashing on a missing class.
#
# Upstream sync is manual: if llama.cpp fixes a bug in ModelBase / TextModel
# / the Qwen3 handling, port it here by hand. Do NOT re-vendor the full
# file — the non-Qwen branches are intentionally deleted.
...
# Mistral-common multimodal dataset statistics are kept as module-level
# constants so surviving Mmproj / vision-encoder code that references them
# continues to resolve. The mistral_common import chain itself is gone
# because no surviving model class touches it.
    ...
    def __init__(self, dir_model: Path, ftype: gguf.LlamaFileType, fname_out: Path, *, is_big_endian: bool = False,
        ...
        if self.is_mistral_format:
            raise NotImplementedError(
                "Mistral-format input is not supported in the LunaVox build of this converter"
            )
...
class TextModel(ModelBase):
    ...
    # _set_vocab_mistral removed with the non-Qwen model classes — LunaVox
    # never dispatches to a Mistral-format code path. See Phase 2 cleanup.
...
class QwenModel(TextModel):
    """Kept purely for the ``token_bytes_to_string`` / ``bpe`` static helpers
    that ``TextModel.get_vocab_base_pre`` delegates to when processing a
    tiktoken-style ``mergeable_ranks`` tokenizer. The class is still
    registered so its full ``set_vocab`` path stays available if a caller
    ever feeds an old-style Qwen1 checkpoint, but LunaVox's own conversion
    pipeline only dispatches to ``Qwen3Model`` via ``Qwen3ForCausalLM``."""
...
def main() -> None:
    ...
    # LunaVox-only build: Mistral-format / mmproj / multimodal paths are
    # out of scope — the vendored branches that dispatched to PixtralModel /
    # MistralModel / MistralMoeModel have been removed along with their
    # class definitions.
    if args.mistral_format:
        raise NotImplementedError(
            "--mistral-format is not supported in the LunaVox build of this converter"
        )
    if args.mmproj:
        raise NotImplementedError(
            "--mmproj is not supported in the LunaVox build of this converter"
        )
