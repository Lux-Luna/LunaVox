...
class WindowsBuilder(Builder):
    def post_build(self):
        # Shared metadata.json copy first; then the MinGW-specific runtime
        # DLLs that no other toolchain ships by default.
        super().post_build()
            ...
            for d in self.ctx.env.get("PATH", "").split(";"):
                ...
                for dll in dlls:
                    ...
                    if src.exists() and not (self.ctx.build_dir / dll).exists():
                        ...
                        console.print(f"[dim][build] Bundled compiler runtime: {dll}[/dim]")
