...
> [!TIP]
> **资源优化与文件精简**:
> 1.  **直接删除**: 如果您的使用场景不涉及从录制的 `.wav` 音频现场提取音色特征（即：仅使用内置发音人、基础合成、Voice Design 或已预处理特征的 JSON 模式），您可以**直接删除**模型目录下的 `qwen3_tts_speaker_encoder.fp16.onnx` 和 `qwen3_tts_codec_encoder.fp16.onnx`。
> 2.  **引擎优化**: LunaVox 引擎已实现“可选延迟加载”。只有在执行 `.wav` 模式的复刻时，引擎才会尝试读取这两个文件。若文件缺失，引擎会提示错误，但不会影响其他 15 条链路的正常运行。
> 3.  **收益**: 这样做可以为每个模型目录节省约 **130MB - 140MB** 的磁盘空间，并略微提升启动时的扫描速度。
